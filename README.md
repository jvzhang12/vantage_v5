...
`Recall` should be treated mainly as the memory-grounding role/view over selected Attention resources, not as the main top-level product noun. In the current API, `recall` carries the older narrowed vetted subset and `working_memory` remains a compatibility alias for it. `ChatService.search_context()` remains the transitional retrieval and vetting input, but normal chat generation now consumes recalled context through an internal Attention/Recall context handoff adapter so the generation prompt, final traces, role projection, and `/api/chat` `working_memory_view` share one compact role/provenance source. The handoff keeps Memory Trace aliases and public summaries safe; legacy search context remains available for fallback and diagnostic parity comparison. The React Vantage view consumes this contract as an inspection view with compact role sections, provenance, and execution summaries.
