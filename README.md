...
`Recall` should be treated mainly as the memory-grounding role/view over selected Attention resources, not as the main top-level product noun. In the current API, `recall` carries the older narrowed vetted subset and `working_memory` remains a compatibility alias for it. `ChatService.search_context()` is still an independent transitional retrieval path; final traces now include an internal Attention/Recall context handoff that compares that legacy Recall output with selected Attention resources without changing generation. The trace-only role projection and `/api/chat` `working_memory_view` are built from that handoff plus TurnPlan execution/write summaries, and they are meant to answer “what did Vantage use?” rather than expose private model reasoning. The React Vantage view consumes this contract as an inspection view with compact role sections, provenance, and execution summaries.
...
The visible best-guess disclosure should be handled through compact answer metadata, such as the `Intuitive Answer` badge, rather than a repeated sentence in the response body.
...
To run the current Vantage v6 app locally:

```bash
cd "/Users/eden/Documents/Vantage v6"
python3 -m venv .venv
