...
API `recall` is currently narrower: the vetted retrieved subset from Memory Trace and Library sources. Legacy `working_memory` is still emitted as an alias for that recalled subset, not the full product concept. `ChatService.search_context()` remains the transitional retrieval source feeding candidate selection and vetting, but normal chat generation now consumes the vetted Recall set through an internal Attention/Recall context handoff adapter. Final-response traces include diagnostic parity between handoff-derived generation context and legacy search/Recall ids, while the Attention/Recall role projection and public Working Memory view are built from that same handoff so they share one compact role/provenance source.
