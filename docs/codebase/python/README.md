...
- `context_handoff.py` builds the internal Attention/Recall/Working Memory read model from finalized turn payloads and adapts the vetted Recall set into the generation context shape so prompts, traces, and the public Working Memory view share one compact role/provenance source.
