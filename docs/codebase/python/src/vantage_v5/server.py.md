...
- `_chat_turn_response()` also persists final-response observability after route-level post-processing, appending a `final_response` section to the low-level chat trace or writing a final-response trace when no service trace exists.
...
- `/api/accounts` creates local username/password accounts when auth/profile mode is enabled. If `VANTAGE_V5_ACCOUNT_CREATION_CODE` is configured, the submitted `access_code` must match before username validation or account creation proceeds. The route rejects usernames that collide with configured auth users or existing local accounts, hashes passwords before writing `state/accounts.json`, seeds the new user's private storage root, initializes an empty profile-local calendar file only when no global calendar events file is configured, and returns a login cookie so the user lands in their own durable session immediately. Canonical defaults remain read-through references rather than copied account files.
...
- `/api/chat` now uses the narrowed TurnPlan surface-authority helper during route-level post-processing, passing request-level write hints such as `memory_intent` alongside the finalized payload. That helper decides whether artifact mutation compilation is disabled and whether operational surface payloads should be built for the finalized surface action.
- `/api/chat` now applies TurnPlan write-intent compatibility projection after execution and post-processing but before final trace persistence. Explicit save/publish/memory/protocol writes can therefore carry additive `surface_invocation.write_intent` / `write_effects` metadata, and generic no-write-looking surface fields are upgraded only as a final response projection rather than as write authority.
- `/api/chat` preserves final Whiteboard `open_only` handoffs as UI-only actions during post-processing: TurnPlan surface authority disables artifact mutation compilation, local semantic save/publish execution, and operational payload replacement for those turns so selected saved material can open in Whiteboard without being recast as a calendar/task proposal or write.
- `/api/chat` also preserves additive close-visible-surface `surface_action` directives during post-processing through TurnPlan surface authority, so commands like `close the whiteboard` or `close the calendar` hide UI context without becoming deletes, drafts, graph writes, or calendar/task proposals.
- `/api/chat` skips artifact mutation compilation and operational surface-payload creation for preserve-visible-surface turns too, so keep/leave-open commands cannot become calendar/task foregrounding, proposals, or other write side effects during route-level post-processing.
- `/api/chat` now records the final browser-facing response contract after post-processing, including selected attention resources, navigator selection, surface invocation, active surface id, surface payloads, artifact actions, final write fields, visible artifacts, response mode, and answer basis, while preserving the existing low-level trace fields for compatibility. The final-response request snapshot keeps the raw message and caps persisted history to the same last-six entries used by the existing trace contract.
...
- `/api/concepts/open` accepts `record_id` as the generic saved-item alias, keeps `concept_id` for compatibility, and delegates the selected-item reopen flow to `DraftArtifactLifecycle`. Its `graph_action` now truthfully uses the generic `open_saved_item_into_workspace` label while still mirroring `concept_id` for older clients; missing saved items are normalized into a clean 404, and protocol records are rejected as 400s because protocols are guidance/editing objects rather than whiteboard drafts. The route also surfaces additive `source_provenance` and `opened_copy` payloads so canonical/reference read-through material opened into Whiteboard remains traceable as a local editable copy.
