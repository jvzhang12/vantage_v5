...
- `/api/chat` preserves final Whiteboard `open_only` handoffs as UI-only actions during post-processing: artifact mutation compilation is skipped for those turns so selected saved material can open in Whiteboard without being recast as a calendar/task proposal or write.
- `/api/chat` also preserves additive close-visible-surface `surface_action` directives during post-processing and skips artifact mutation compilation for those turns, so commands like `close the whiteboard` or `close the calendar` hide UI context without becoming deletes, drafts, graph writes, or calendar/task proposals.
- `/api/chat` skips artifact mutation compilation for preserve-visible-surface turns too, so keep/leave-open commands cannot become calendar/task proposals or other write side effects during route-level post-processing.
