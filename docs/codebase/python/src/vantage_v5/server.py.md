...
- `/api/chat` now records the final browser-facing response contract after post-processing, including selected attention resources, navigator selection, surface invocation, active surface id, surface payloads, artifact actions, final write fields, visible artifacts, response mode, and answer basis, while preserving the existing low-level trace fields for compatibility.
