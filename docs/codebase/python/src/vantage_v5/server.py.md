...
- `/api/chat` now uses the narrowed TurnPlan surface-authority helper during route-level post-processing. That helper decides whether artifact mutation compilation is disabled and whether operational surface payloads should be built for the finalized surface action.
- `/api/chat` preserves final Whiteboard `open_only` handoffs as UI-only actions during post-processing: TurnPlan surface authority disables artifact mutation compilation and operational payload replacement for those turns so selected saved material can open in Whiteboard without being recast as a calendar/task proposal or write.
