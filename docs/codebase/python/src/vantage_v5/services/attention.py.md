...
- `AttentionTurn.select()` normalizes the Navigator's selection or chooses a small deterministic fallback set. Selection normalization accepts either public resource ids or compact candidate ids, and when a Whiteboard open includes concepts, visible operational surfaces, same-title opened copies, derivative action artifacts, and their source artifact, it makes the source artifact the primary open target.
