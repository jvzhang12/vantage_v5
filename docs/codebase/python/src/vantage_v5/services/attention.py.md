...
- `AttentionEngine.prepare_turn()` indexes candidate resources from the current runtime and user-scoped operational providers, resolving operational calendar/task dates from the configured app/user timezone when no explicit test date is supplied.
