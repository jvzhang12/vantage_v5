...
- Control-panel normalization now treats the model output as untrusted structured input: unsupported action types are dropped, `apply_protocol` actions must name a supported protocol kind, `close_surface` and `preserve_surface` actions get normalized targets/confidence, non-protocol actions have `protocol_kind` reset to `null`, and any structured actions suppressed by no-op preservation can be carried forward as compact `suppressed_actions` metadata for downstream denial receipts.
...
- `apply_control_panel_open_intent_fallback()` is a narrow deterministic Navigator/control-panel fallback for saved/open-material lookup turns and explicit `remember that ...` memory turns. When the model selected a saved artifact but omitted the explicit open signal, this helper adds `surface_to_open="whiteboard"` plus an `open_whiteboard` control-panel action. When a clear memory request otherwise drifted toward an operational surface, it adds a `remember` action and clears the surface open. Ordinary artifact Q&A and keep/leave-open visible-surface turns remain chat/context-only. Preserve/keep-open normalization strips conflicting open/close/draft actions from active controls, but records stripped structured draft/open/close controls in `control_panel.suppressed_actions` so TurnPlan can explain denied drafts without reinterpreting raw user text.
