...
- Keep protocol action validation deterministic while preserving the target architecture: the Navigator decides when to press `apply_protocol`; deterministic code validates and resolves the supported protocol kinds.
- Run the protocol interpreter and apply structured protocol upsert decisions without adding keyword-based protocol learning.
...
- `interpret_and_apply()` owns protocol learning/upsert side effects. It only acts on a structured `ProtocolInterpreter` decision, writes through `ConceptStore.upsert_protocol()`, returns the `ExecutedAction`, and merges the updated protocol back into the concept records used for the turn.
