...
- `ScenarioLabService` receives the active runtime scope plus canonical/experiment roots from the server and uses them for created branch, comparison-artifact, Memory Trace, selected-record, and retrieval provenance payloads instead of deriving experiment/durable status from path segments.
