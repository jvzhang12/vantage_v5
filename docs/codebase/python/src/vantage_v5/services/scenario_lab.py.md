...
- `_save_comparison_artifact()` renders the comparison Markdown, neutralizes outcome-biased comparison titles when needed, reuses that neutral title for the saved artifact body heading, stores the artifact through `ArtifactStore` with `type="scenario_comparison"` as a stable marker, and writes durable scenario metadata including the branch ids, namespace, and comparison artifact id into the Markdown body.
- `ScenarioLabTurn.to_dict()` returns the API payload, including the navigator decision, a concise summary plus recommendation, the saved branch metadata, the comparison artifact, unified `recall` / `working_memory`, `learned`, and `response_mode` fields, plus a `created_record` compatibility alias derived from the first learned item for UI reuse.
...
- `ScenarioLabTurn`: API-facing turn payload for Scenario Lab responses.
- `ScenarioLabService`: coordinates retrieval, generation, persistence, and tracing.
- `_openai_build_scenario()`: calls OpenAI to build the structured scenario plan.
...
- Branch labels are slugified, generic comparisons can break out of the active workspace namespace, workspace ids are made unique before saving, and both branch workspaces and the comparison artifact now carry parseable scenario metadata in Markdown rather than a hidden store.
...
- The comparison artifact is promoted into the existing saved-item UI path via both `learned` and its `created_record` compatibility alias, while the branch workspaces remain separate workspace records.
