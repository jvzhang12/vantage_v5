...
- The orchestrator now builds TurnPlan draft authority before normal chat execution and passes that verdict into `ChatService.reply()`. Pending Whiteboard offers/drafts are allowed only when structured draft/offer authority exists; open-only, close, preserve, and artifact-Q&A no-write turns force chat/no-draft execution and honest no-draft receipts if a model emits a Whiteboard signal anyway.
