...
- Route public Memory Trace DTOs, Memory Trace-derived candidate rows, Attention state, grouped memory aliases, and vetting ids through `public_context_projection.py` so `/api/chat` exposes bounded aliases without raw trace bodies, prompt text, or prompt-derived storage ids.
...
- Public Memory Trace records and Memory Trace-derived candidate/note rows are reduced to safe views by the shared projection helper: the current-turn record uses the `current-turn` alias, selected/recalled prior-turn records use bounded `memory_trace:prior-turn-N` aliases, and raw `body` / `content` / storage ids stay out of the browser payload.
