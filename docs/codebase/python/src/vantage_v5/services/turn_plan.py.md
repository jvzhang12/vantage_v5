...
- `write_projection` records intended write kinds, authority/source, actual write categories, and intent/effect agreement from structured fields only. It recognizes semantic policy `artifact_save` / `artifact_publish`, control-panel write actions, `memory_intent="remember"`, protocol upserts, existing write-intent plan fields, and finalized write effects.
- Compatibility projection is additive and post-execution: explicit save/publish/memory/protocol writes can annotate `surface_invocation.write_intent` and `surface_invocation.write_effects`, preserving legacy fields where useful while avoiding final traces that look like generic no-write chat.
