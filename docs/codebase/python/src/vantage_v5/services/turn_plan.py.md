...
- Validation warnings are non-authoritative diagnostics only. They flag contradictions such as selected context opening Whiteboard without explicit open authority, `open_only` carrying writes, preserve/close turns carrying side effects, visible-artifact Q&A writing without explicit write authority, draft surface mismatches, operational active-surface/payload mismatches, and calendar/task mutations that are not proposal-only.
