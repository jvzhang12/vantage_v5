...
- Capture retrieval selection, explicit UI surface intent, write/draft intent, a trace-only write ledger, visible context, side-effect policy, semantic/protocol summaries, execution policy, compatibility links, and trace-only validation warnings without changing runtime behavior.
