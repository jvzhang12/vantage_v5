...
- Memory-write-authority coverage verifies that request-level `memory_intent="remember"` and structured control-panel remember actions allow memory candidates, while hard no-write surface authority and missing/unsafe content deny them. It also verifies memory write effects without structured authority produce validation diagnostics.
