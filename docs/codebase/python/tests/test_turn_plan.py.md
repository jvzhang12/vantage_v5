...
- Concept-write-authority coverage verifies that existing meta `create_concept` / `create_revision` candidates and structured control-panel concept actions are allowed only when persisted candidate fields are available, revision targets are present when needed, and hard no-write surface authority is absent. It also verifies that missing/unsafe concept content and concept effects without authority are denied or diagnosed.
- Protocol-write-authority coverage verifies that structured protocol interpreter `upsert_protocol` candidates are allowed only when safe protocol content and a concrete protocol id/kind target are present, the existing protocol-write policy allows the write, and hard no-write surface authority is absent. It also verifies that denied protocol effects produce validation diagnostics.
