...
- Operational-proposal-authority coverage verifies that proposed calendar/task candidates are allowed only when structured proposal candidates exist, safe content/targets are available, every candidate has exact `status="proposed"`, all candidates require confirmation, and hard no-write surface authority is absent. It also verifies that hard no-write plus a proposal candidate produces validation diagnostics, non-confirmation-gated candidates are denied, and missing/arbitrary/committed statuses are not treated as valid proposed mutations.
...
- Final response trace payloads include `final_response.turn_plan.write_ledger`, `final_response.turn_plan.validation`, and the bounded `final_response.working_memory_view` contract for open-only grounding summaries.
