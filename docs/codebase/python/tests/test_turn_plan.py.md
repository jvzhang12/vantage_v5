...
- Protocol-write-authority coverage verifies that structured protocol interpreter `upsert_protocol` candidates are allowed only when safe protocol content and a concrete protocol id/kind target are present, the existing protocol-write policy allows the write, and hard no-write surface authority is absent. It also verifies that protocol writes get their own ledger/projection category even though protocol persistence uses the concept store, and that denied protocol effects produce validation diagnostics.
