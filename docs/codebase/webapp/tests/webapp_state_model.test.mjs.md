...
- Verify auth-state helper behavior for expired protected sessions versus ordinary failed login attempts.
- Verify workspace-context scoping for chat requests.
- Verify deictic whiteboard reopen resolution for follow-ups that should reopen a uniquely recalled durable item instead of drafting against the currently open whiteboard, while excluding protocol guidance records from reopen targets.
...
- Cover the distinction between “can return to whiteboard” and “whiteboard is in scope,” plus the backend-matched pending-draft carry contract, including the narrow explicit-whiteboard carveout, `fresh whiteboard` phrasing, hidden-whiteboard exclusion for fresh chat-originated draft requests, hidden active draft revision routing from Chat, explicit chat-only overrides, forced scope overrides, and dirty-snapshot defaults that preserve transient drafts.
