...
class ArtifactActionPlanner:
    ...
    def plan_for_turn(
        ...
        *,
        ...
        if not _looks_like_artifact_mutation(message):
        ...
        if not calendar_is_visible and not _looks_calendar_specific(message):
            return ArtifactActionPlan(artifact_actions=[])
        ...
        if not action.artifact_actions and action.assistant_message:
            return action
...
def execute_artifact_action(
    *,
    ...
    if action.get("artifact_kind") != "calendar":
        raise ValueError("Only calendar artifact actions are supported.")
    if action.get("operation") not in CALENDAR_OPERATIONS:
    ...
    if operation == "create_event":
        result = calendar_provider.create_event(
        ...
        result = calendar_provider.soft_cancel_event(str(payload.get("event_id") or ""))
    else:
        updates = payload.get("updates") if isinstance(payload.get("updates"), dict) else {}
        result = calendar_provider.update_event(str(payload.get("event_id") or ""), updates)
    accepted = {
        **action,
        "status": ARTIFACT_ACTION_STATUS_ACCEPTED,
        "preview": {"before": result.get("before"), "after": result.get("after")},
        "result": result,
        "summary": result.get("summary") or action.get("summary") or "Calendar updated.",
...
def action_graph_payload(action: dict[str, Any]) -> dict[str, Any]:
    return {
        "type": f"calendar_{action.get('operation') or 'artifact_action'}",
...
def _proposal_assistant_message(action: dict[str, Any]) -> str:
    summary = str(action.get("summary") or "I found a calendar change to make.").strip()
