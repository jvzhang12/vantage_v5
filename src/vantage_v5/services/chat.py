...
class ChatService:
    ...
    def reply(
        ...
        *,
        ...
        if protocol_action is not None and suppress_auto_graph_writes and not suppress_protocol_writes:
        ...
        if suppress_auto_graph_writes:
            meta = MetaDecision(
                action="no_op",
                rationale=(
                    "TurnPlan marked this turn as no-write, so Vantage did not create a durable graph record."
                ),
            )
            executed_action = None
        elif protocol_action is not None:
            meta = MetaDecision(
                action="no_op",
                rationale=(
...
def build_final_response_trace_payload(
    *,
    ...
    working_memory_view = response_payload.get("working_memory_view")
    if not isinstance(working_memory_view, dict):
        working_memory_view = build_working_memory_view_payload(
            request_payload=request_payload,
            response_payload=response_payload,
