...
from vantage_v5.services.public_context_projection import public_attention_selection
from vantage_v5.services.public_context_projection import public_memory_trace_list
from vantage_v5.services.public_context_projection import public_memory_trace_record
from vantage_v5.services.public_context_projection import public_turn_interpretation_dict
from vantage_v5.services.public_context_projection import public_vetting_payload
...
def assemble_turn_interpretation_payload(parts: TurnInterpretationParts) -> dict[str, Any]:
    ...
    if parts.navigation.attention_selection is not None:
        payload["attention_selection"] = public_attention_selection(parts.navigation.attention_selection)
    return payload
...
def _turn_interpretation_payload(value: TurnInterpretationParts | dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    if isinstance(value, dict):
        return public_turn_interpretation_dict(value)
    return None
...
def assemble_chat_turn_body(parts: ChatTurnBodyParts) -> dict[str, Any]:
    ...
    trace_notes = public_memory_trace_list(parts.trace_notes)
    candidate_trace_notes = public_memory_trace_list(parts.candidate_trace_notes)
    concept_cards = public_memory_trace_list(parts.concept_cards)
    saved_notes = public_memory_trace_list(parts.saved_notes)
    vault_notes = public_memory_trace_list(parts.vault_notes)
    candidate_concepts = public_memory_trace_list(parts.candidate_concepts)
    candidate_saved_notes = public_memory_trace_list(parts.candidate_saved_notes)
    candidate_vault_notes = public_memory_trace_list(parts.candidate_vault_notes)
    candidate_memory_results = public_memory_trace_list(parts.candidate_memory)
    working_memory = public_memory_trace_list(parts.working_memory)
    recall_details = public_memory_trace_list(parts.recall_details)
    selected_attention_resources = public_memory_trace_list(parts.selected_attention_resources or [])
    selected_memory = _turn_memory_payload(saved_notes, vault_notes)
    candidate_memory = _turn_memory_payload(candidate_saved_notes, candidate_vault_notes)
    memory_trace_record = public_memory_trace_record(
    ...
    payload = {
        ...
        "vetting": public_vetting_payload(parts.vetting),
        "mode": parts.mode,
